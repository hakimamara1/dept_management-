...
| `purchase_invoices` | Invoice header from OCR or manual entry | `UNIQUE(invoice_number, supplier_id)`; `status` ∈ `Pending Review, Approved, Rejected`; `approved_at` is `NULL` until `approveInvoice()` sets it once, at the exact moment of approval — powers the read-only view page's "تاريخ الاعتماد" field. `invoice_amount` is **always** `SUM(purchase_invoice_items.total_price)`, kept live by `recalculateInvoiceTotal` on every item add/edit/delete — never read `invoice_amount` as if it were the raw OCR figure. `ocr_header_total` holds that raw OCR figure instead, frozen at import, reference-only (`NULL` for manual entries — there's no OCR figure to compare against) — see `business-rules.md`. `source` ∈ `ocr, manual` — which creation path produced this invoice; purely informational, doesn't change how it's processed |
...
| `invoice_attachments` | Photos of the physical invoice | Backup documentation only, never read by business logic — `file_path` is relative to `src/data/uploads/`, served statically at `GET /uploads/<file_path>` |
